...
type Initial = { name: string; country: string; interests: string[] };

const wrap: React.CSSProperties = { background: "#f5ead0", minHeight: "100vh", display: "flex", justifyContent: "center" };
const card: React.CSSProperties = { width: "100%", maxWidth: 390, minHeight: "100vh", background: "#f5ead0" };
...
const chip = (a: boolean): React.CSSProperties => ({ background: a ? "#2e8b57" : "#fff9f0", border: `2px solid ${a ? "#2e8b57" : "#e8c99a"}`, color: a ? "#fff" : "#8a7560", borderRadius: 20, padding: "7px 14px", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" });
...
export default function EditTravelerForm({ userEmail, initial }: { userEmail: string; initial: Initial }) {
  ...
  function onDelete(e: React.FormEvent<HTMLFormElement>) {
    if (!confirm("旅行者プロファイルを削除する？取り消せないわよ。")) e.preventDefault();
  }
        ...
        <div style={{ padding: "20px 20px 100px" }}>
          ...
          <form action={action}>
            ...
            <div style={{ marginBottom: 18 }}>
              ...
              <input id="country" name="country" required list="country-suggestions" defaultValue={initial.country} style={input} />
            ...
            <div style={{ marginBottom: 24 }}>
              ...
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                {INTEREST_OPTIONS.map((t) => (
                  <button key={t} type="button" onClick={() => setInterests((s) => toggle(s, t))} style={chip(interests.includes(t))}>
                    {t}
                  </button>
                ))}
              </div>
